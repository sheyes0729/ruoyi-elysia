...
const buildDeptOptions = (deptList: DeptTreeItem[], depth = 0): { label: string; value: number }[] => {
...
const fetchDepts = async () => {
  ...
  try {
    ...
    if (res.data?.code === 200) {
      ...
      parentDeptOptions.value = [{ label: '顶级部门', value: 0 }, ...buildDeptOptions(res.data.data)]
...
<template>
  <n-card title="部门管理" :bordered="false" class="dept-page">
    <template #header-extra>
      <n-space>
        <n-button type="primary" size="small" @click="openAdd">新增部门</n-button>
        <n-button type="primary" size="small" :disabled="!selectedDept" @click="openEdit">
          编辑
        </n-button>
        <n-popconfirm @positive-click="handleDelete">
          <template #trigger>
            <n-button type="error" size="small" :disabled="!selectedDept">删除</n-button>
          </template>
          确认删除该部门？
        </n-popconfirm>
      </n-space>
    ...
    <n-space :size="16" align="start">
      <n-card title="部门列表" :bordered="false" size="small" style="width: 350px">
        <n-tree
          :data="depts"
      ...
      <n-card title="部门详情" :bordered="false" size="small" style="flex: 1">
        ...
        <nDescriptions v-else :column="2" label-placement="left" bordered>
          <nDescriptionsItem label="部门ID">{{ selectedDept.deptId }}</nDescriptionsItem>
          <nDescriptionsItem label="部门名称">{{ selectedDept.deptName }}</nDescriptionsItem>
          <nDescriptionsItem label="状态">
            <n-tag :type="selectedDept.status === '0' ? 'success' : 'error'" size="small">
              {{ selectedDept.status === '0' ? '正常' : '停用' }}
            </n-tag>
          </nDescriptionsItem>
          <nDescriptionsItem label="排序">{{ selectedDept.orderNum }}</nDescriptionsItem>
        </nDescriptions>
      </n-card>
    </n-space>
  </n-card>
  ...
  >
    <n-form :model="formData" label-placement="left" label-width="100">
      <n-form-item label="上级部门">
        <n-select v-model:value="formData.parentId" :options="parentDeptOptions" />
      </n-form-item>
      <n-form-item label="部门名称" path="deptName">
        <n-input v-model:value="formData.deptName" placeholder="请输入部门名称" />
      </n-form-item>
      <n-form-item label="排序">
        <n-input-number v-model:value="formData.orderNum" :min="0" :max="9999" />
      </n-form-item>
