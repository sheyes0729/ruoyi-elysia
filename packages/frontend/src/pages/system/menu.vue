...
const buildMenuOptions = (menuList: MenuItem[], depth = 0): { label: string; value: number }[] => {
...
const handleDelete = async () => {
  ...
  const res = await api.api.system.menu.remove.delete({
    query: { ids: [selectedMenu.value.menuId] },
  })
  if (res.data?.code === 200) {
...
const renderLabel = (node: MenuItem) => {
  return h(NSpace, { size: 4 }, () => [
    ...
    h(
      NTag,
      { size: 'small', type: getMenuTypeTagType(node.menuType) },
...
<template>
  <n-card title="菜单管理" :bordered="false" class="menu-page">
    <template #header-extra>
      <n-space>
        <n-button type="primary" size="small" @click="openAdd">新增菜单</n-button>
        <n-button type="primary" size="small" :disabled="!selectedMenu" @click="openEdit">
          编辑
        </n-button>
        <n-popconfirm @positive-click="handleDelete">
          <template #trigger>
            <n-button type="error" size="small" :disabled="!selectedMenu">删除</n-button>
          </template>
          确认删除该菜单？
        </n-popconfirm>
      </n-space>
    ...
    <n-space :size="16" align="start">
      <n-card title="菜单列表" :bordered="false" size="small" style="width: 400px">
        <n-tree
          :data="menus"
      ...
      <n-card title="菜单详情" :bordered="false" size="small" style="flex: 1">
        ...
        <n-descriptions v-else :column="2" label-placement="left" bordered>
          <n-descriptions-item label="菜单ID">{{ selectedMenu.menuId }}</n-descriptions-item>
          <n-descriptions-item label="菜单名称">{{ selectedMenu.menuName }}</n-descriptions-item>
          <n-descriptions-item label="菜单类型">
            <n-tag :type="getMenuTypeTagType(selectedMenu.menuType)" size="small">
              {{ getMenuTypeLabel(selectedMenu.menuType) }}
            </n-tag>
          </n-descriptions-item>
          <n-descriptions-item label="路由路径">{{ selectedMenu.path }}</n-descriptions-item>
          <n-descriptions-item label="组件路径">{{ selectedMenu.component }}</n-descriptions-item>
          <n-descriptions-item label="权限标识">{{ selectedMenu.perms }}</n-descriptions-item>
          <n-descriptions-item label="状态">
            <n-tag :type="selectedMenu.status === '0' ? 'success' : 'error'" size="small">
              {{ selectedMenu.status === '0' ? '正常' : '停用' }}
            </n-tag>
          </n-descriptions-item>
          <n-descriptions-item label="排序">{{ selectedMenu.orderNum }}</n-descriptions-item>
        </n-descriptions>
      </n-card>
    </n-space>
  </n-card>
  ...
  >
    <n-form :model="formData" label-placement="left" label-width="100">
      ...
      <n-form-item label="菜单类型">
        <n-select v-model:value="formData.menuType" :options="menuTypeOptions" />
      </n-form-item>
      ...
      <n-form-item label="权限标识">
        <n-input v-model:value="formData.perms" placeholder="如: system:user:list" />
      </n-form-item>
